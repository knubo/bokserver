...
class RegnSession {
	...
	function __construct($db) {
		...
		if(!$db->table_exists(AppConfig :: prefix() ."sessions")) {
            $query = 'CREATE TABLE '.AppConfig :: prefix() .'sessions (
    ...
    function sessao_read( $aKey ) {
	   ...
	   $prep = $this->db->prepare("SELECT DataValue FROM ".AppConfig :: prefix() ."sessions WHERE SessionID=?");
       ...
       if(sizeof($res) > 0) {
		   ...
		   $prep = $this->db->prepare       	
             ("INSERT INTO ".AppConfig :: prefix() ."sessions (SessionID, LastUpdated, DataValue)
                       VALUES (?, NOW(), '')");
           $prep->bind_params("s", $aKey);
	...
	function sessao_write( $aKey, $aVal ) {
       $prep = $this->db->prepare       	
             ("UPDATE ".AppConfig :: prefix() ."sessions SET DataValue = ?, LastUpdated = NOW() WHERE SessionID = ?");
	...
	function sessao_destroy( $aKey ) {
       $prep = $this->db->prepare       	
             ("DELETE FROM ".AppConfig :: prefix() ."sessions WHERE SessionID = ?");
       $prep->bind_params("s", $aKey);
       $prep->execute($prep);
       return TRUE;
 	}
	...
	function sessao_gc( $aMaxLifeTime ) {
       $prep = $this->db->prepare       	
             ("DELETE FROM ".AppConfig :: prefix() ."sessions WHERE UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(LastUpdated) > ?");
